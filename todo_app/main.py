...
# Функция для подсчёта завершённых задач
@app.get("/completed-items-count")
def get_completed_items_count(db: Session = Depends(get_db)):
    completed_items_count = db.query(TodoItemModel).filter(TodoItemModel.completed == True).count()
    return {"completed_items_count": completed_items_count}
